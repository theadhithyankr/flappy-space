...
export default class GameScene extends Phaser.Scene {
  ...
  drawBackground() {
    ...
    for (let i = 0; i < 32; i++) {
        ...
        const angle2 = ((i + 0.5) / 32) * Math.PI * 2;
        const radius = Math.max(width, height);
        graphics.lineTo(width/2 + Math.cos(angle1) * radius, height/2 + Math.sin(angle1) * radius);
        graphics.lineTo(width/2 + Math.cos(angle2) * radius, height/2 + Math.sin(angle2) * radius);
        graphics.fillPath();
    }
  ...
  drawPaylineIndicators(graphics, startX, startY, reelW, rowH, gap) {
    const boxLeft = startX - reelW / 2 - 10;
    const frameW = reelW * 3 + gap * 2;
    const boxRight = boxLeft + frameW + 20;
    ...
    const leftX = boxLeft - 22;
    const rightX = boxRight + 22;

    indicators.forEach(ind => {
        // Draw connecting lines
        graphics.lineStyle(3, 0xffdf00, 0.7);
        graphics.beginPath();
        graphics.moveTo(leftX + 16, startY + ind.yOffset);
        graphics.lineTo(boxLeft, startY + ind.yOffset);
        graphics.strokePath();

        graphics.beginPath();
        graphics.moveTo(rightX - 16, startY + ind.yOffset);
        graphics.lineTo(boxRight, startY + ind.yOffset);
        graphics.strokePath();
        ...
        // Add text immediately rather than in buildUi
        this.add.text(leftX, startY + ind.yOffset, ind.num, {fontFamily: 'Impact', fontSize: 18, color: '#ffffff'}).setOrigin(0.5);
        this.add.text(rightX, startY + ind.yOffset, ind.num, {fontFamily: 'Impact', fontSize: 18, color: '#ffffff'}).setOrigin(0.5);
    });
  }
